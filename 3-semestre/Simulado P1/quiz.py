class Quiz:
    ...
    def iniciarQuiz(self):
        ...
        print("\n--- Iniciando Quiz ---")
        i = 0
        while i < len(self.perguntas):
            self.perguntar(i)
            self.responder(i)
            i+=1
        print(f"{self.nome} fez {self.pontuacao} pontos")

    def perguntar(self, questao):
        print(self.perguntas[questao]["enunciado"])
        for alt in self.perguntas[questao]["alternativas"]:
            print(alt)

    def responder(self, pergunta):
        resposta = input("Entre com a letra da resposta: ").upper()
        if resposta == self.perguntas[pergunta]["resposta"]:
            print("Acertou!")
            self.pontuacao += 1
        else:
            print(f"Errou. A correta era: {self.perguntas[pergunta]['resposta']}")
